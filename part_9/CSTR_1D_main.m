...
for tol=tols
    ...
    for i=1:size(Fs,2)
        ...
        params = [beta,k0,EaR,CAin,CBin,Tin,V];
        args = {F, [beta,k0,EaR,CAin,CBin,Tin,V]};
        [T_local,X_local,r_local,h_local, info_local] = ClassicRK4_adaptive(@CSTR_1D_fJ,tspan,h0,x0,abstol,reltol,args);
...
for tol=tols
    ...
    for i=1:size(Fs,2)
        ...
        params = [beta,k0,EaR,CAin,CBin,Tin,V];
        args = {F, [beta,k0,EaR,CAin,CBin,Tin,V]};
        [T_local,X_local,e,r_local,h_local, info_local] = DOPRI54_adaptive(@CSTR_1D,tspan,h0,x0,abstol,reltol,butcher,args);
...
for tol=tols
    ...
    for i=1:size(Fs,2)
        ...
        sol_3D_45 = ode45(@(t,x) CSTR_1D(t,x,F,params),tspan, x0, opts);
...
for tol=tols
    ...
    for i=1:size(Fs,2)
        ...
        opts = odeset('RelTol',tol,'AbsTol',tol);
        sol_3D_45 = ode15s(@(t,x) CSTR_1D(t,x,F,params),tspan, x0, opts);
