...
[T,X,X_real,e_l,e_g,r_out,h_out,info] = ClassicRK4TE_adaptive(@TestEquation, ...
    @Exponential, tspan, h0, x0,abstol,reltol, args);

figure('Renderer', 'painters', 'Position', [680   600   600   500])
subplot(4,1,[1,2])
yyaxis left
plot(T,X)
hold on
...
print(gcf,'6_3_TestEquation','-dpdf','-r0')

%% 1.4.Evolution of the error vs. stepsize
clc
close all
tspan = [0 30];
tols = logspace(-4, -9, 15);
h0 = 0.1;
...
for tol=tols
    i = i+1;
    [T,X,X_real,e_l,e_g,r_out,h_out,info] = ClassicRK4TE_adaptive(@TestEquation, ...
    @Exponential, tspan, h0, x0,tol,tol, args);
...
loglog(tols,e_ls_ex)
hold on
loglog(tols, 0.2*tols.^(0.9),'LineStyle','--','Color','k')
xlabel('tol')
ylabel('local error')
legend('RK4','O(tol^{0.9})','Location','NorthWest')

set(gcf,'Units','Inches');
pos = get(gcf,'Position');
set(gcf,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])
print(gcf,'6_3_localerror','-dpdf','-r0')

figure()
loglog(tols,e_gs_ex)
hold on
loglog(tols, .03*tols.^(3/4),'black','LineStyle','--','Color','k')
xlabel('tol')
ylabel('global error')
legend('RK4','O(tol^{3/4})','Location','NorthWest')

set(gcf,'Units','Inches');
pos = get(gcf,'Position');
set(gcf,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])
print(gcf,'6_3_globalerror','-dpdf','-r0')
