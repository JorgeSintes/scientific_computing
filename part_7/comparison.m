...
for tol=tols
    tols_legend = [tols_legend, 'DOPRI54, tol = ' + string(tol), 'ode45, tol = ' + string(tol)];
end
...
print(gcf,'7_6_mu_1_5','-dpdf','-r0')

infos_csv = [tols;infos];
infos_45_csv = [tols;infos_45];
csvwrite('7_6_mu_1_5.csv',infos_csv,0,1);
csvwrite('7_6_mu_1_5_ode45.csv',infos_45_csv,0,1);
...
for tol=tols
    tols_legend = [tols_legend, 'DOPRI54, tol = ' + string(tol), 'ode15s, tol = ' + string(tol)];
end
...
pos = get(gcf,'Position');
set(gcf,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])
print(gcf,'7_6_mu_15','-dpdf','-r0')

infos_csv = [tols;infos];
infos_15_csv = [tols;infos_15];
csvwrite('7_6_mu_15.csv',infos_csv,0,1);
csvwrite('7_6_mu_15_ode15s.csv',infos_15_csv,0,1);
...
for tol=tols
    tols_legend = [tols_legend, 'DOPRI54, tol = ' + string(tol), 'ode45, tol = ' + string(tol), 'ode15s, tol = ' + string(tol)];
end
...
for tol=tols
    ...
    for i=1:size(Fs,2)
        ...
        [T_local,X_local,e_local,r_local,h_local,info_local] = DOPRI54_adaptive(@CSTR_3D,tspan,h0,x0,abstol,reltol, butcher,args);
    ...
    for i=1:size(Fs,2)
        ...
        params = [beta,k0,EaR,CAin,CBin,Tin,V];
        args = {F, [beta,k0,EaR,CAin,CBin,Tin,V]};
        [T_local,X_local,e_local,r_local,h_local,info_local] = DOPRI54_adaptive(@CSTR_1D,tspan,h0,x0,abstol,reltol,butcher,args);
    ...
    csvwrite('7_6_3D.csv',infos_3D_csv,0,1);
    csvwrite('7_6_3D_45.csv',infos_3D_45_csv,0,1);
    csvwrite('7_6_3D_15.csv',infos_3D_15_csv,0,1);
    
    infos_1D_csv = [tols;infos_1D];
    infos_1D_45_csv = [tols;infos_1D_45];
    infos_1D_15_csv = [tols;infos_1D_15];
    csvwrite('7_6_1D.csv',infos_1D_csv,0,1);
    csvwrite('7_6_1D_45.csv',infos_1D_45_csv,0,1);
    csvwrite('7_6_1D_15.csv',infos_1D_15_csv,0,1);
end
